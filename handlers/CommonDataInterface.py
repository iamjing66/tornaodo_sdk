...
from handlers.kbeServer.Editor.response import response_account, response_mail, response_class, response_resversion, \
    response_other, response_update, response_sis, response_fullview, response_collect, response_global
...
class PostInterfaceRequest(BaseHandler):
    ...
    def post(self):
        ...
        if "langage" in json_data.keys():
            language_str = json_data["ch"]

        logging.info("PostInterfaceRequest -> UID[%i],opencode[%i],subcode[%i],username[%s]" % (
        UID, opencode, subcode, username))
        DB = DBManager()
        if opencode == 0:
            ...
            if subcode == 3 or subcode == 4 or subcode == 33:
                phone_code = application.App.Redis_SMS.GetCode(username, subcode)
            json_back = response_account.Transactions_Register(DB, subcode, UID, username, data,
                                                               phone_code)  # AccountInst.DoCodeVer(subcode,UID,username,data,phone_code)
            ...
            logging.info("login - get serverAddress = " + url)
            # 顶号
            ...
            json_back = AvatarXREditorInst.Transactions_Code(subcode, UID, username, language_str, data)
