...
from handlers.kbeServer.Editor.Interface import interface_mail, interface_project, interface_obj, interface_global,interface_solr,interface_wit
...
def BuyNew(DB,self_uid,uid,cid,lid,tlong):
    logging.info("赠送课程数据 :" + str(self_uid) + "-" + str(uid) + "-" + str(cid) + "-" + str(uid) + "-" + str(lid) + "-" + str(tlong))
...
def LessonUD(DB,cid,uid,lid,status):
    ...
    for _lid in _lids:
        ...
        sql = "select ID from sys_course_res where CID = "+str(cid) + " and uid = "+str(uid) + " and mlessonID = " + str(_lid)
        data = DB.fetchone(sql, None)
        _id = 0
        if data:
            _id = int(data[0])
        ...
        if _id == 0:
            sql = "insert into sys_course_res (cid,uid,mlessonID,flag) values ('"+str(cid)+"','"+str(uid)+"','"+str(_lid)+"','"+str(status)+"')"
        else:
