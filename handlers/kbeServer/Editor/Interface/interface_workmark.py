...
def DoWorkLogData(DB, self_uid, wid, lid, uid, sis_cid, PID, ipage, ilenght, ctype):
    ...
    if _exist:
        if ipage == 0:
            if PID == 0:
                sql = "select t1.log,t1.`Date`,t2.UserName,t2.NickName,t2.UID,t1.ID,(SELECT COUNT(ID) FROM " + table_name + " WHERE PID = t1.ID) AS HF,(SELECT COUNT(ID) FROM " + table_name1 + " WHERE TID = t1.ID AND ZAN = 1) AS ZAN,(select zan from " + table_name1 + " where UID = " + str(
                    self_uid) + " and TID  = t1.ID) as IsZan from " + table_name + " as t1 inner join tb_userdata as t2 on t1.uid = t2.uid and t1.log != '' AND T1.PID = 0 order by t1.`Date` desc;"
                sql1 = "select UID,NickUrl from tb_userdata where uid in ( select UID from " + table_name + " WHERE LOG != '' AND PID = 0 GROUP BY UID );"
            else:
                sql = "select t1.log,t1.`Date`,t2.UserName,t2.NickName,t2.UID,t1.ID,(SELECT COUNT(ID) FROM " + table_name + " WHERE PID = t1.ID) AS HF,(SELECT COUNT(ID) FROM " + table_name1 + " WHERE TID = t1.ID AND ZAN = 1) AS ZAN,(select zan from " + table_name1 + " where UID = " + str(
                    self_uid) + " and TID  = t1.ID) as IsZan from " + table_name + " as t1 inner join tb_userdata as t2 on t1.uid = t2.uid and t1.log != '' AND T1.PID = " + str(
                    PID) + " order by t1.`Date` desc;"
                sql1 = "select UID,NickUrl from tb_userdata where uid in ( select UID from " + table_name + " WHERE LOG != '' AND PID = " + str(PID) + " GROUP BY UID );"
        else:
            if PID == 0:
                sql = "select t1.log,t1.`Date`,t2.UserName,t2.NickName,t2.UID,t1.ID,(SELECT COUNT(ID) FROM " + table_name + " WHERE PID = t1.ID) AS HF,(SELECT COUNT(ID) FROM " + table_name1 + " WHERE TID = t1.ID AND ZAN = 1) AS ZAN,(select zan from " + table_name1 + " where UID = " + str(
                    self_uid) + " and TID  = t1.ID) as IsZan from " + table_name + " as t1 inner join tb_userdata as t2 on t1.uid = t2.uid and t1.log != '' AND T1.PID = 0 order by t1.`Date` desc limit " + str(
                    (ipage - 1) * ilenght) + "," + str(ilenght) + ";"
                sql1 = "select UID,NickUrl from tb_userdata where uid in ( select UID from " + table_name + " WHERE LOG != '' AND PID = 0 GROUP BY UID );"
            else:
                sql = "select t1.log,t1.`Date`,t2.UserName,t2.NickName,t2.UID,t1.ID,(SELECT COUNT(ID) FROM " + table_name + " WHERE PID = t1.ID) AS HF,(SELECT COUNT(ID) FROM " + table_name1 + " WHERE TID = t1.ID AND ZAN = 1) AS ZAN,(select zan from " + table_name1 + " where UID = " + str(
                    self_uid) + " and TID  = t1.ID) as IsZan from " + table_name + " as t1 inner join tb_userdata as t2 on t1.uid = t2.uid and t1.log != '' AND T1.PID = " + str(
                    PID) + " order by t1.`Date` desc limit " + str((ipage - 1) * ilenght) + "," + str(ilenght) + ";"
                sql1 = "select UID,NickUrl from tb_userdata where uid in ( select UID from " + table_name + " WHERE LOG != '' AND PID = " + str(PID) + " GROUP BY UID );"
        ...
        if data:
            ...
            for minfo in list_data:
                ...
                if _base_string == "":
                    _base_string = minfo_list[0] + "@lyyym@" + str(minfo_list[1]) + "@lyyym@" + minfo_list[2] + "@lyyym@" + minfo_list[3] + "@lyyym@" + str(minfo_list[4]) + "@lyyym@" + str(
                            minfo_list[5]) + "@lyyym@" + str(minfo_list[6]) + "@lyyym@" + str(_ZAN_NUM) + "@lyyym@" + str(_ZAN)
                else:
                    _base_string = _base_string + "@lyyyym@" + minfo_list[0] + "@lyyym@" + str(minfo_list[1]) + "@lyyym@" + minfo_list[2] + "@lyyym@" + minfo_list[3] + "@lyyym@" + str(
                            minfo_list[4]) + "@lyyym@" + str(minfo_list[5]) + "@lyyym@" + str(minfo_list[6]) + "@lyyym@" + str(_ZAN_NUM) + "@lyyym@" + str(_ZAN)
