...
def ClassSchedule(DB, pam):
    ...
    sql_str = "select * from (SELECT T3.*,T4.ID as tid,T4.`start` as tstart,t4.`end` as tend from(select t1.*,t2.UserName from (select id,chapter,courses,`start`,`end`,courseId,MlessonID,UID,PID FROM `events` WHERE scheduleId = (select scheduleId from tb_class where CID = " + str(
        classid) + ") and istask = 0 and mlessonId != 0 ORDER BY `end`) as t1 LEFT join tb_userdata as t2 on t1.UID = T2.UID) as T3 inner join `events` AS T4 ON t3.CourseID = t4.CourseID and t3.MLessonID = t4.MLessonID and t3.UID = t4.UID and t3.id = t4.parent_id AND T3.PID = t4.PID AND T4.istask = 1 and T4.mlessonId != 0 and T4.scheduleId = (select scheduleId from tb_class where CID = " + str(
        classid) + ")) t5 order by `end`"
    ...
    if data:
        ...
        for minfo in list_data:
            if _cback != "":
                _cback = _cback + "*" + str(minfo[0]) + "," + str(minfo[1]) + "," + str(minfo[2]) + "," + str(minfo[3]) + "," + str(minfo[4]) + "," + str(minfo[5]) + "," + str(minfo[6]) + "," + str(
                        minfo[7]) + "," + str(minfo[8]) + "," + str(minfo[9]) + "," + str(minfo[10]) + "," + str(minfo[11]) + "," + str(minfo[12])
            else:
                _cback = str(minfo[0]) + "," + str(minfo[1]) + "," + str(minfo[2]) + "," + str(minfo[3]) + "," + str(minfo[4]) + "," + str(minfo[5]) + "," + str(minfo[6]) + "," + str(
                        minfo[7]) + "," + str(minfo[8]) + "," + str(minfo[9]) + "," + str(minfo[10]) + "," + str(minfo[11]) + "," + str(minfo[12])
