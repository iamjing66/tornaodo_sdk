...
def GetMail(DB, uid, data):
    ...
    sql = "select t2.ID,t2.msg_writedate,t2.msg_title,t2.msg_body,t3.RFLAG,t2.msg_talker,t2.msg_channel from (SELECT * FROM (select ID,msg_writedate,msg_title,msg_body,msg_readed,msg_talker,msg_channel from tb_message where msg_channel = 0 or msg_channel = " + str(
        uid) + " and msg_writedate > '" + str(userdate) + "' order by ID desc) T1 where ID NOT IN (select `MID` from tb_message_rd where uid = " + str(
        uid) + " and DFLAG = 1) ) t2 left join tb_message_rd t3 on t2.ID = t3.`MID` AND T3.UID = " + str(uid) + " order by t2.ID desc limit " + str(_Index) + ",8"
    ...
    if data:
        for minfo_data in data:
            ...
            if _cback == "":
                ...
                _cback = _cback + "!" + str(minfo_data[0]) + "`" + str(minfo_data[1]) + "`" + str(minfo_data[2]) + "`" + str(minfo_data[3]) + "`" + str(minfo_data[5]) + "`" + str(_flag) + "`" + str(
                        minfo_data[6])
        json_data["code"] = "1"
...
def InsertEmilData(DB, jddata, uid):
    ...
    sql_str = "INSERT INTO " + table_project + " (msg_channel, msg_writedate, msg_title, msg_body, msg_readed, msg_talker) Values ('" + str(msg_channel) + "','" + str(localtime) + "','" + str(
        msg_title) + "','" + str(msg_body) + "',0,0)"
    DB.edit(sql_str, None)
