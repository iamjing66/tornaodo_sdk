...
def FB(db, self_uid, uid, pid, template):
    sql = "update tb_project set Template = {0} where UID = {1} and PID = {2}".format(str(template), str(uid), str(pid))
    if db.edit(sql, None):
        return 1
    return 0
...
def SetPublish(db, self_uid, uid, pid, publish):
    sql = "update tb_project set Publish = {0} where UID = {1} and PID = {2}".format(str(publish), str(uid), str(pid))
    if db.edit(sql, None):
        return 1
    return 0
...
def SetFrom(db, uid, pid, From, FromPam):
    sql = "Update tb_project set `From`={0},FromPam = \'{1}\' where uid = {2} and pid = {3}".format(str(From), str(FromPam), str(uid), str(pid))
    db.edit(sql, None)
    return 1
...
def CopyMyProjectToAccount(db, self_uid, pid, pname, username, cmode):
    ...
    if data__p:
        ...
        ProjectToDB(db, NPID, P_UID, 0, data__p)
