...
def PC_lOGON(DB, Username, json_data):
    ...
    _SEND = Phone + "$" + str(Power) + "$" + str(UPower) + "$" + str(MainAccount) + "$" + TheName + "$" + TheSchool + "$" + TheClass + "$" + identity + "$" + str(COMID) + "$" + str(
        PassDate) + "$" + AccountDesc + "$" + BCBag + "$" + str(organization) + "$" + str(distributor) + "$" + str(AccountType) + "$" + str(uversion) + "$" + str(makec) + "$" + str(
        AccountSource) + "$" + class_str + "$" + str(sclass) + "$" + str(fabricator) + "$" + str(adminType) + "$" + str(AccountPower) + "$" + str(VipPower) + "$" + str(
        VipDate) + "$" + Package_Msg + "$" + str(uid) + "$" + Res_Msg + "$" + Scene_Msg + "$" + str(gmstate) + "$" + str(save_status)
    # print("_SEND",_SEND)
...
def VR_LOGIN(DB, VR_USERNAME, json_data):
    ...
    if data:
        ...
        _userdata_string = str(data[3]) + "^" + data[0] + "^" + str(data[1]) + "^" + str(data[4]) + "^" + str(data[5]) + "^" + str(data[6]) + "^" + str(data[7]) + "^" + str(data[10]) + "^" + str(
                data[11]) + "^" + str(data[12]) + "^" + str(data[13]) + "^" + str(data[9]) + "^" + str(data[14]) + "^" + str(data[15]) + "^" + str(data[8]) + "^" + str(data[16])
        vipdate = int(data[16])
        UPower = int(data[4])
        # login
        # 记录redis缓存
...
def Register(DB, uid, username, data, phone_recode):
    ...
    userName, passWord, power, upower, desc, EndDate, mainAccount, organization, distributor, accountType, AccountSource, AccountOther, NickName, sfz, phone, school, _class, AccountPower, '', '', '',
    '', '0'))
    if create_user:
        json_data["code"] = 1
        json_data["msg"] = "注册成功"
