...
def InsertPayOrder( _order, _uid, _username, _from, _paytype, _price, _pam, _IP, _AppType, DB):
    sql = "INSERT INTO `tb_pay_order`(`order`,`UID`,`USERNAME`,`FROM`,`PAYTYPE`,`PRICE`,`PAM`,`IP`,APPTYPE)VALUES ( '" + _order + "','" + str(_uid) + "','" + _username + "','" + _from + "'," + str(_paytype) + "," + str(_price) + ",'" + _pam + "','" + _IP + "'," + str(_AppType) + " );"
    DB.edit(sql, None)
