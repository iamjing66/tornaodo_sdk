...
def GetWorkSQLFromTypeNew(itype, self_uid, pam, **kwargs):
    if itype == 0:
        ...
        if not kwargs["d_class"]:
            sql = "select T1.*,T2.username,0 from tb_workmarket t1 inner join tb_userdata t2 on t1.UID = t2.UID and t1.boutique = %s;" % \
                  kwargs["course_level"]
        else:
            sql = "select w.id, w.WID, w.Name, w.Platform, w.Stars, w.Pid, w.price, w.`desc`, w.State, w.NewPid, w.CreateDate, w.UID,w.Vision, w.Sid, w.Version, w.Game, w.GameStage, w.SNUM, w.identity, w.SNAME, w.ct, e.price1, e.price2, e.FREE, e.stime, e.etime, w.freestatus, w.boutique, e.sort, e.flag, w.Plat, t.UserName, 0 from tb_workmarket as w inner join tb_eservices_workmarket as e inner join tb_userdata as t on w.wid = e.wid and w.uid = e.uid and t.UID = %s and e.organization_id = t.distributor and w.boutique = %s;" % (
...
def Data_Work_Base(uid, wid, target, db, call_type, sh=False, d_class=False, self_uid=0):
    json_back = None
    ...
    sql = GetWorkSQLFromType(3, uid, arrPam, d_class=d_class, p_uid=self_uid)
    result = db.fetchone(sql, None)
    if result:
        # GM审核
        if sh:
...
def UpdateWorkFlag(db, wid, uid, flag):
    ...
    if flag == 1:
        sign = 3
    sql = "update tb_workbag set State = " + str(sign) + ",Version = Version + 1 where WID = " + str(
        wid) + " and UID = " + str(uid)
...
def UpdateToDB(data, uid, wID, db, target):
    ...
    if result:
        ...
        if len(data) == 23:
            sql = "Update " + table_name + " set WID = " + str(data[0]) + ", Name='" + str(
                data[1]) + "', Platform=" + str(data[2]) + ", Stars=" + str(data[3]) + ", Pid=" + str(
                data[4]) + ", price=" + str(data[5]) + ", `desc`='" + str(data[6]) + "', State=" + str(
                data[7]) + ", NewPid=" + str(data[8]) + ", CreateDate=" + str(data[9]) + ", Vision=" + str(
                data[11]) + ", Sid=" + str(data[12]) + ", Version=" + str(data_version) + ", Game=" + str(
                data[14]) + ", GameStage=" + str(data[15]) + ", SNUM='" + str(data[16]) + "', `identity`='" + str(
                data[17]) + "', SNAME='" + str(data[18]) + sql_else + ",price1=" + str(data[20]) + ",price2=" + str(
                data[21]) + ",Free = " + str(data[22]) + ",Plat='1'" + " where UID = " + str(uid) + " and WID =" + str(
                wID)
        else:
            sql = "Update " + table_name + " set WID = " + str(data[0]) + ", Name='" + str(
                data[1]) + "', Platform=" + str(data[2]) + ", Stars=" + str(data[3]) + ", Pid=" + str(
                ...
                data[14]) + ", GameStage=" + str(data[15]) + ", SNUM='" + str(data[16]) + "', `identity`='" + str(
                data[17]) + "', SNAME='" + str(data[18]) + sql_else + ",price1=" + str(data[20]) + ",price2=" + str(
                data[21]) + ",Free = " + str(data[22]) + ",Plat=" + str(data[23]) + " where UID = " + str(
                uid) + " and WID =" + str(wID)
    else:
        if len(data) == 23:
            sql = "Insert INTO " + table_name + " (WID,`Name`,Platform,Stars,Pid,price,`desc`,State,NewPid,CreateDate,UID,Vision,Sid,Version,Game,GameStage,SNUM,`identity`,SNAME,ct,price1,price2,Free,Plat) values (" + str(
                wID) + ",'" + str(data[1]) + "'," + str(data[2]) + "," + str(data[3]) + "," + str(data[4]) + "," + str(
                data[5]) + ",'" + str(data[6]) + "'," + str(data[7]) + "," + str(data[8]) + "," + str(
                data[9]) + "," + str(uid) + "," + str(data[11]) + "," + str(data[12]) + "," + str(data[13]) + "," + str(
...
def BuyFlag(db, self_uid, uid, pid):
    ...
    _version = 0
    _pid = 0
    sql = "select PDate,version,pid from tb_project where P_uid = " + str(uid) + " and ParentPid = " + str(
        pid) + " and UID = " + str(self_uid) + " limit 0,1"
    result = db.fetchone(sql, None)
...
def PAYPAM_WorkMK(paydata, db):
    ...
    if _id == 0 or _price2 <= 0:
        ...
        params = str(wid) + "@" + str(b_uid) + "@" + str(organization) + "@" + str(distributor) + "@" + json_data[
